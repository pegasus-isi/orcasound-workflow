...
class OrcasoundWorkflow():
    ...
    # --- Init ---------------------------------------------------------------------
    def __init__(self, sensors, start_date, end_date, max_files, case, dagfile="workflow.yml"):
    ...
    def create_replica_catalog(self):
        ...
        if (self.case in {"edge", "edge-cloud"}):
            prefix_location = "file:///home/panorama/public_html/ooi-data/{}"
        else:
            prefix_location = "http://10.100.100.192/~panorama/ooi-data/{}"
        
        # Add s3 files as deep lfns
        for f in self.s3_files["Key"]:
            self.rc.add_replica("condorpool", f, prefix_location.format(f))
            ...
            for ts in self.s3_files[self.s3_files["Sensor"] == sensor]["Timestamp"].unique():
                ...
                for job_files in np.array_split(sensor_ts_files, num_of_splits):
                    ...
                    inference_job = (Job("inference", _id="predict_{0}_{1}_{2}".format(sensor, ts, counter), node_label="inference_{0}_{1}_{2}".format(sensor, ts, counter))
                                        .add_args("-i wav/{0}/{1} -s {0} -t {1} -m /opt/ooi/model/model.pkl -o predictions_{0}_{1}_{2}.json".format(sensor, ts, counter))
                                        .add_inputs(*wav_files)
                ...
                merge_job_ts = (Job("merge", _id="merge_{0}_{1}".format(sensor, ts), node_label="merge_{0}_{1}".format(sensor, ts))
                                    ...
                                    .add_pegasus_profiles(label="merge_jobs")
                                )
...
if __name__ == '__main__':
    ...
    parser.add_argument("-c", "--case", metavar="STR", default="edge", choices=["edge", "cloud", "edge-cloud"], help="Deployment case")

    args = parser.parse_args()
    if not args.end_date:
        args.end_date = args.start_date + timedelta(days=1)
    
    workflow = OrcasoundWorkflow(sensors=args.sensors, start_date=args.start_date, end_date=args.end_date, max_files=args.max_files, case=args.case, dagfile=args.output)
